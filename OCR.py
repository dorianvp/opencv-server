...
class OCR:
    ...
    def get_index_coordinate(image, results, VISIBILITY_THRESHOLD=0.5, PRESENCE_THRESHOLD=0.5):
        height, width, _ = image.shape
        r = [
            int(results.multi_hand_landmarks[0].landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP].x * width),
    ...
    def get_index_dip_coordinates(image, results, VISIBILITY_THRESHOLD=0.5, PRESENCE_THRESHOLD=0.5):
        """
        Return the coordinates of the index finger DIP
        """
        height, width, _ = image.shape
        r = [
            int(results.multi_hand_landmarks[0].landmark[mp_hands.HandLandmark.INDEX_FINGER_DIP].x * width),
            int(results.multi_hand_landmarks[0].landmark[mp_hands.HandLandmark.INDEX_FINGER_DIP].y * height),
...
def put_ocr_boxes(boxes, frame, height, crop_width=0, crop_height=0, view_mode=1):
    ...
    for i in range(n_boxes):
        if int(boxes['conf'][i]) > 0:  # Filter out weak confidences
            ...
            cv2.rectangle(frame, (x + crop_width, y + crop_height), (x + crop_width + w, y + crop_height + h), (0, 255, 0), 2)
            # Optionally, put the text above the rectangle
            cv2.putText(frame, boxes['text'][i], (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    while True:
        ...
        for i in range(1, len(pts)):
            if pts[i - 1] is None or pts[i] is None:
                continue
            thick = int(np.sqrt(len(pts) / float(i + 1)) * 10)
            cv2.line(frame, (pts[i - 1][0], pts[i - 1][1]), tuple(pts[i][0:2]), (0, 0, 200), thick)
            pts.pop()
        ...
        if index_coordinates:
            ...
            # frame, text = put_ocr_boxes(ocr.boxes, frame, img_hi, cropx, cropy, view_mode)
